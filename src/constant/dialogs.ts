export const dialog1 = [
  {
    idx: 1,
    props: 5,
    name: '교수님',
    text: '내 연구생이 기숙사를 가서 돌아오지 않는다..',
  },
  {
    idx: 2,
    props: 5,
    name: '교수님',
    text: '거기 우리에게 꼭 필요한 방호복이 있는데...',
  },
  {
    idx: 3,
    props: 5,
    name: '교수님',
    text: '4층에 산다고 했던것 같은데..혹시 어디인지 아는가?',
  },
];

export const dialog2 = [
  {
    idx: 1,
    props: 7,
    name: '오타쿠',
    text: '히사시부리~~',
  },
  {
    idx: 2,
    props: 7,
    name: '오타쿠',
    text: '미르미쿤,이런 상황에선 패기가 중요하네.',
  },
  {
    idx: 3,
    props: 0,
    name: '오타쿠',
    text: '미르미쿤의 답변 여하에 따라 내가 아이템을 주겠네',
  },
  {
    idx: 4,
    props: 0,
    name: '오타쿠',
    text: '내 지원서 내용을 맞춰보게나',
  },
];

export const dialog3 = [
  {
    idx: 1,
    props: 1,
    name: '미르미',
    text: '(숨을 헐떡이며) 교수님! 왜 아직 이 교실에 계세요!',
  },
  {
    idx: 2,
    props: 1,
    name: '미르미',
    text: '제가 구해드릴게요!.',
  },
  {
    idx: 3,
    props: 6,
    name: '익명의 교수',
    text: '(기침하며)콜록,,콜록,,,',
  },
  {
    idx: 4,
    props: 6,
    name: '익명의 교수',
    text: '나는 아직 대피할 수 없네',
  },
  {
    idx: 5,
    props: 6,
    name: '익명의 교수',
    text: '나는 30년 평생 교수로 살아있다네..',
  },
  {
    idx: 6,
    props: 6,
    name: '익명의 교수',
    text: '아직 한 명이 출석 체크를 안 한 것 같네..',
  },
  {
    idx: 7,
    props: 6,
    name: '익명의 교수',
    text: `에타에도 올라와있군..  '미르미' 출첵했나요??`,
  },
];

export const dialog4 = [
  {
    idx: 1,
    props: 8,
    name: '최강록',
    text: '헤헴...미디어를 1년 간 끊었건만..기어코 나에게 왔구만',
  },
  {
    idx: 2,
    props: 1,
    name: '미르미',
    text: '현재 굶주리고 있는 사람이 많아요. 저를 도와주세요!',
  },
  {
    idx: 3,
    props: 8,
    name: '최강록',
    text: '허허 나야 강록이...',
  },
  {
    idx: 4,
    props: 1,
    name: '미르미',
    text: '네 그러니까 빨리 도와주세요!',
  },
  {
    idx: 5,
    props: 8,
    name: '최강록',
    text: '난 그 건물이 아니면 요리를 안 해.',
  },
  {
    idx: 6,
    props: 1,
    name: '미르미',
    text: '탄소 6개, 근데 이제 수소를 6개 곁들인..건물 말씀하시는 거죠?',
  },
];

export const dialog5 = [
  {
    idx: 1,
    props: 9,
    name: '행정실 직원',
    text: '제가 그저께 입사해서...',
  },
  {
    idx: 2,
    props: 9,
    name: '행정실 직원',
    text: '이럴 때 연락해야 할 대학이 있다고 하셨는데..',
  },
  {
    idx: 3,
    props: 9,
    name: '행정실 직원',
    text: '저희와 비슷한 대학교 세 군데가 있다고 하시던데..',
  },
  {
    idx: 4,
    props: 9,
    name: '행정실 직원',
    text: '어느 대학들인지 알고 계신가요?',
  },
];

export const dialog6 = [
  {
    idx: 1,
    props: 0,
    name: ' ',
    text: "어라..?\n'미르미'의 아이템들이 반응하기 시작했다..!",
  },
  {
    idx: 2,
    props: 0,
    name: ' ',
    text: '미르미는 정보처리기사로 전직했다!',
  },
  {
    idx: 3,
    props: 0,
    name: '[미션]',
    text: '기사님! \n율전을 점령하려는 버그 원숭이를 모두 디버깅하고 영웅이 되어주세요!',
  },
  {
    idx: 4,
    props: 0,
    name: '[미션]',
    text: '무작위로 움직이는 버그 원숭이들을 모두 터치해주세요! 단, 귀여운 숭이는 공격해서는 안됩니다.',
  },
];
